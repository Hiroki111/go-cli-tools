...
func setupAPI(t *testing.T) (string, func()) {
	...
	server := httptest.NewServer(newMux(tempTodoFile.Name()))
	...
	for i := 1; i < 3; i++ {
		...
		r, err := http.Post(server.URL+"/todo", "application/json", &body)
		if err != nil {
			t.Fatal(err)
		}
...
func TestAdd(t *testing.T) {
	...
	t.Run("CheckAdd", func(t *testing.T) {
		...
		if response.Results[0].Task != taskName {
			t.Errorf("Expected %q, got %q", taskName, response.Results[0].Task)
		}
	})
}
...
func TestDelete(t *testing.T) {
	...
	t.Run("CheckDelete", func(t *testing.T) {
		...
		if len(response.Results) != 1 {
			t.Errorf("Expected 1 item, got %d", len(response.Results))
		}

		expectedTask := "Task number 2"
		if response.Results[0].Task != expectedTask {
			t.Errorf("Expected %q, got %q.", expectedTask, response.Results[0].Task)
		}
	})
}
